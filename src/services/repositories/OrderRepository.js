...
export default class OrderRepository extends BaseRepository {
	...
	_checkStockToUpdate = async (arrayProducts) => {
		...
		for (const item of arrayProducts) {
			...
			if (product.stock < item.quantity)
				throw new Error("Product does not have the requested stock quantity.");
	...
	_validateDoc = async (data) => {
		...
		if (!buyer || !items.length || !total_to_pay || !total_items)
			throw new Error("Missing fields");

		if (!buyer.first_name || !buyer.last_name || !buyer.phone || !buyer.address || !buyer.user)
			throw new Error("Missing fields on 'buyer'");
	};
}
